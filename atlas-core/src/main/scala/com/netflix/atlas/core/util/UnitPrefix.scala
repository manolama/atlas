...
object UnitPrefix {
  ...
  val min   = UnitPrefix("min",      "min",   60.0)
  val hour  = UnitPrefix("hr",       "hour",  3600.0)
  val day   = UnitPrefix("day",      "day",   hour.factor * 24)
  val mon   = UnitPrefix("mon",      "month", day.factor * 30)
  ...
  val durationBigPrefixes: List[UnitPrefix] = List(year, mon, day, hour, min, sec)
  ...
  private def withinRange(prefix: UnitPrefix, v: Double, f: Double): Boolean = {
    val a = math.abs(v)
