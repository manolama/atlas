...
object Ticks {
  ...
  private val durationValueTickSizes = {
    ...
    val majorMultiples = List(
      ...
      List(1, 2, 5, 10, 15, (86400 * 30)),
      List(1, 2, 3, 4, 6, (86400 * 30 * 365))
    )
  ...
  private def durationTicks(v1: Double, v2: Double, t: (Double, Double, Int)): List[ValueTick] = {
    ...
    while (pos <= end) {
      ...
      if (v >= v1 && v <= v2) {
        ...
        val (labelFmt, _) = labelFormat(prefix, major)
        val label = prefix.format(v, labelFmt)
        ticks += ValueTick(v, 0.0, pos % minorPerMajor == 0, Some(label))
      }
      pos += 1
    ...
    if (useOffset) {
      ...
      val newFormat = labelFormat(offsetPrefix, major)._1

      ts.map(t => t.copy(offset = base, labelOpt = Some(offsetPrefix.format(t.v - base, newFormat)))
      )
    } else ts
  }
